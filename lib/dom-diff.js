...
(function () {
    ...
    /** Truncate a string for display in op descriptions. */
    ...
    /** Last URL path segment, with query/fragment stripped — for image-swap descriptions. */
    function urlFilename(url) {
        if (!url) return '(unknown)';
        try {
            const u = new URL(url, 'https://x.invalid');
            const seg = u.pathname.split('/').filter(Boolean).pop() || '(unknown)';
            return seg;
        } catch (e) {
    ...
    function makeTextReplace(origEl, editEl, originalHtml, ctx) {
        ...
        return {
            ...
            intent: 'Change text content of ' + origEl.tagName.toLowerCase() +
                (origEl.id ? '#' + origEl.id : ''),
            // Deterministic natural-language description of the visible outcome.
            // Used by (a) the receiving Claude agent in Step 6 verification — refetch
            // the live page and confirm the described change is visible, and (b)
            // future client-side review UX. NEVER generated by an LLM — pulled
            // verbatim from the op fields above so it cannot hallucinate.
            expected_visible_change:
                'Change <' + origEl.tagName.toLowerCase() + '> text from "' +
                shortText(oldInner) + '" to "' + shortText(newInner) + '"',
    ...
    function makeImageSwap(origEl, editEl, originalHtml, ctx) {
        ...
        return {
            ...
            expected_visible_change:
                'Replace image at ' + selector + ' (was "' +
                urlFilename(originalSrc) + '", now "' + urlFilename(filenameHint || newSrc) + '")',
            match_strategies: strategies
        };
    }
    ...
    function makeAttrChange(origEl, editEl, attr, before, after, originalHtml, ctx) {
        ...
        return {
            ...
            expected_visible_change:
                'Set ' + attr + ' on ' + selector + ' from "' +
                shortText(before == null ? '(unset)' : before, 50) + '" to "' +
                shortText(after == null ? '(removed)' : after, 50) + '"',
            match_strategies: strategies
        };
    }
    ...
    function makeSubtreeReplace(origEl, editEl, originalHtml, ctx, note) {
        ...
        return {
            ...
            expected_visible_change:
                'Replace ' + (origEl.tagName.toLowerCase()) + ' subtree at ' + selector +
                (note ? ' (' + note + ')' : ''),
            match_strategies: strategies
        };
    }
